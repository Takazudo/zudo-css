...
<CssPreview
  ...
  <div class="form__group">
    ...
    <input class="form__input form__input--error" type="password" placeholder="Enter password">
...
- **Server-rendered apps** where HTML and CSS ship separately without a build tool to scope styles
- **Multi-team projects** where a shared naming convention prevents collisions across teams
- **CSS without build tools** where you need predictable naming without framework-level scoping
- **Legacy projects** being incrementally refactored toward better CSS architecture

BEM is less necessary when using:

- **CSS Modules** — class names are locally scoped by the build tool
- **CSS-in-JS** (styled-components, Emotion) — styles are scoped to components by default
- **Tailwind CSS** — utility-first approach removes the need for naming conventions entirely

Even in these environments, understanding BEM's principles (flat selectors, clear naming, component-scoped styles) improves how you structure CSS.
