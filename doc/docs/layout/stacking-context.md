...
Without `isolation: isolate` on `.card`, the `z-index: -1` background would fall behind the card entirely, because it would be compared against elements outside the card in the parent stacking context.
